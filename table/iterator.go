...
func (itr *Iterator) seekFrom(key []byte, whence int) {
	...
	idx := sort.Search(len(itr.t.blockIndex), func(idx int) bool {
		...
		return MemTabKeyComparator(ko.key, key) > 0
	})
	if idx == 0 {
...
func (s *ConcatIterator) Seek(key []byte) {
	...
	if !s.reversed {
		idx = sort.Search(len(s.tables), func(i int) bool {
			return MemTabKeyComparator(s.tables[i].Biggest(), key) >= 0
		})
	} else {
		n := len(s.tables)
		idx = n - 1 - sort.Search(n, func(i int) bool {
			return MemTabKeyComparator(s.tables[n-1-i].Smallest(), key) <= 0
		})
	}
...
func (itr *blockIterator) Seek(key []byte, whence int) {
	...
	for itr.Init(); itr.Valid(); itr.Next() {
		k := itr.Key()
		if MemTabKeyComparator(k, key) >= 0 {
...
func (itr *blockIterator) parseKV(h header) {
	...
	itr.val = append(itr.val[:0], itr.data[itr.pos:itr.pos+uint32(h.vlen)]...)
...
func (itr *blockIterator) Prev() {
	...
	var h header
	util.AssertTruef(itr.pos < uint32(len(itr.data)), "%d %d", itr.pos, len(itr.data))
	itr.pos += uint32(h.Decode(itr.data[itr.pos:]))
	itr.parseKV(h)
	itr.last = h
}
