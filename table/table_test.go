...
func buildTable(t *testing.T, keyValues [][]string) *os.File {
	...
	f, err := util.OpenSyncedFile(filename, true)
	if t != nil {
	...
	for _, kv := range keyValues {
		util.AssertTrue(len(kv) == 2)
		err := b.Add(util.KeyWithTs([]byte(kv[0]), 0), LSMValue{Value: []byte(kv[1]), Meta: 'A', UserMeta: 0})
		if t != nil {
			require.NoError(t, err)
		} else {
			util.Check(err)
	...
	f.Write(b.Finish())
	f.Close()
	f, _ = util.OpenSyncedFile(filename, true)
	return f
}
	...
	for _, n := range []int{99, 100, 101} {
		t.Run(fmt.Sprintf("n=%d", n), func(t *testing.T) {
			...
			for it.Rewind(); it.Valid(); it.Next() {
				v := it.Value()
				k := util.KeyWithTs([]byte(key("key", count)), 0)
...
func TestSeek(t *testing.T) {
	...
	for _, tt := range data {
		it.seek(util.KeyWithTs([]byte(tt.in), 0))
		if !tt.valid {
		...
		require.True(t, it.Valid())
		k := it.Key()
		require.EqualValues(t, tt.out, string(util.ParseKey(k)))
	}
}
...
func TestSeekForPrev(t *testing.T) {
	...
	for _, tt := range data {
		it.seekForPrev(util.KeyWithTs([]byte(tt.in), 0))
		if !tt.valid {
			require.False(t, it.Valid())
			continue
		}
...
func TestIterateFromEnd(t *testing.T) {
	...
	for _, n := range []int{99, 100, 101, 199, 200, 250, 9999, 10000} {
		t.Run(fmt.Sprintf("n=%d", n), func(t *testing.T) {
			...
			defer ti.Close()
			ti.reset()
			ti.seek(util.KeyWithTs([]byte("zzzzzz"), 0)) // Seek to end, an invalid element.
...
func TestTable(t *testing.T) {
	...
	defer ti.Close()
	kid := 1010
	seek := util.KeyWithTs([]byte(key("key", kid)), 0)
	for ti.seek(seek); ti.Valid(); ti.next() {
		k := ti.Key()
		require.EqualValues(t, string(util.ParseKey(k)), key("key", kid))
		kid++
	}
	if kid != 10000 {
	...
	ti.seek(util.KeyWithTs([]byte(key("key", 99999)), 0))
	require.False(t, ti.Valid())

	ti.seek(util.KeyWithTs([]byte(key("key", -1)), 0))
	require.True(t, ti.Valid())
	k := ti.Key()
	require.EqualValues(t, string(util.ParseKey(k)), key("key", 0))
}
...
func TestIterateBackAndForth(t *testing.T) {
	...
	seek := util.KeyWithTs([]byte(key("key", 1010)), 0)
	it := table.NewIterator(false)
	defer it.Close()
	it.seek(seek)
	require.True(t, it.Valid())
	...
	require.True(t, it.Valid())
	k = it.Key()
	require.EqualValues(t, key("key", 1008), string(util.ParseKey(k)))

	it.next()
	it.next()
	require.True(t, it.Valid())
	k = it.Key()
	require.EqualValues(t, key("key", 1010), util.ParseKey(k))

	it.seek(util.KeyWithTs([]byte(key("key", 2000)), 0))
	require.True(t, it.Valid())
	k = it.Key()
	require.EqualValues(t, key("key", 2000), util.ParseKey(k))

	it.prev()
	require.True(t, it.Valid())
	k = it.Key()
	require.EqualValues(t, key("key", 1999), util.ParseKey(k))

	it.seekToFirst()
	k = it.Key()
	require.EqualValues(t, key("key", 0), util.ParseKey(k))
}
...
func TestConcatIteratorOneTable(t *testing.T) {
	...
	require.True(t, it.Valid())
	k := it.Key()
	require.EqualValues(t, "k1", string(util.ParseKey(k)))
	vs := it.Value()
...
func TestConcatIterator(t *testing.T) {
	...
	require.NoError(t, err)
	defer tbl.DecrRef()
	tbl2, err := OpenTable(f2, nil)
	require.NoError(t, err)
	defer tbl2.DecrRef()
	tbl3, err := OpenTable(f3, nil)
	require.NoError(t, err)
	defer tbl3.DecrRef()

	{
		...
		it.Seek(util.KeyWithTs([]byte("a"), 0))
		require.EqualValues(t, "keya0000", string(util.ParseKey(it.Key())))
		vs := it.Value()
		require.EqualValues(t, "0", string(vs.Value))

		it.Seek(util.KeyWithTs([]byte("keyb"), 0))
		require.EqualValues(t, "keyb0000", string(util.ParseKey(it.Key())))
		vs = it.Value()
		require.EqualValues(t, "0", string(vs.Value))

		it.Seek(util.KeyWithTs([]byte("keyb9999b"), 0))
		require.EqualValues(t, "keyc0000", string(util.ParseKey(it.Key())))
		vs = it.Value()
		require.EqualValues(t, "0", string(vs.Value))

		it.Seek(util.KeyWithTs([]byte("keyd"), 0))
		require.False(t, it.Valid())
	}
	{
		...
		it.Seek(util.KeyWithTs([]byte("keyb"), 0))
		require.EqualValues(t, "keya9999", string(util.ParseKey(it.Key())))
		vs := it.Value()
		require.EqualValues(t, "9999", string(vs.Value))

		it.Seek(util.KeyWithTs([]byte("keyb9999b"), 0))
		require.EqualValues(t, "keyb9999", string(util.ParseKey(it.Key())))
		vs = it.Value()
		require.EqualValues(t, "9999", string(vs.Value))

		it.Seek(util.KeyWithTs([]byte("keyd"), 0))
		require.EqualValues(t, "keyc9999", string(util.ParseKey(it.Key())))
		vs = it.Value()
		require.EqualValues(t, "9999", string(vs.Value))
	}
}
...
func TestMergingIterator(t *testing.T) {
	...
	it := NewMergeIterator([]Iterator{it1, it2}, false)
	defer it.Close()
	...
	require.True(t, it.Valid())
	k = it.Key()
	require.EqualValues(t, "k2", string(util.ParseKey(k)))
	vs = it.Value()
...
func TestMergingIteratorReversed(t *testing.T) {
	...
	it := NewMergeIterator([]Iterator{it1, it2}, true)
	defer it.Close()
...
func TestMergingIteratorTakeOne(t *testing.T) {
	...
	t1, err := OpenTable(f1, nil)
	require.NoError(t, err)
	defer t1.DecrRef()
	t2, err := OpenTable(f2, nil)
	require.NoError(t, err)
	defer t2.DecrRef()
...
func BenchmarkRead(b *testing.B) {
	...
	for i := 0; i < n; i++ {
		...
		v := fmt.Sprintf("%d", i)
		util.Check(builder.Add([]byte(k), LSMValue{Value: []byte(v), Meta: 123, UserMeta: 0}))
	}
