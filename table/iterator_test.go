...
func (s *SimpleIterator) Seek(key []byte) {
	key = util.KeyWithTs(key, 0)
	if !s.reversed {
		s.idx = sort.Search(len(s.keys), func(i int) bool {
			return util.CompareKeys(s.keys[i], key) >= 0
		})
	} else {
		n := len(s.keys)
		s.idx = n - 1 - sort.Search(n, func(i int) bool {
			return util.CompareKeys(s.keys[n-1-i], key) <= 0
		})
	}
}

func (s *SimpleIterator) Key() []byte { return s.keys[s.idx] }
func (s *SimpleIterator) Value() LSMValue {
	return LSMValue{
		Value:    s.vals[s.idx],
		UserMeta: 55,
		Meta:     0,
...
func newSimpleIterator(keys []string, vals []string, reversed bool) *SimpleIterator {
	...
	util.AssertTrue(len(keys) == len(vals))
	for i := 0; i < len(keys); i++ {
		k[i] = util.KeyWithTs([]byte(keys[i]), 0)
		v[i] = []byte(vals[i])
	}
...
func getAll(it Iterator) ([]string, []string) {
	...
	for ; it.Valid(); it.Next() {
		k := it.Key()
		keys = append(keys, string(util.ParseKey(k)))
		v := it.Value()
