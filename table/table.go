...
type Table struct {
	...
	// For file garbage collection. Atomic.
	ref        int32
...
func OpenTable(fd *os.File, cksum []byte) (*Table, error) {
	...
	t.mmap, err = util.Mmap(fd, 0, fi.Size(), false)
	if err != nil {
		fd.Close()
		return nil, util.Wrapf(err, "Unable to map file")
	}
...
func (t *Table) loadToRAM() error {
	...
	if err != nil || read != t.tableSize {
		return util.Wrapf(err, "Unable to load file in memory. Table file: %s", t.Filename())
	}
	t.Checksum = sum.Sum(nil)
	return nil
}

func (t *Table) read(off, size int) ([]byte, error) {
	if len(t.mmap) > 0 {
		if len(t.mmap[off:]) < size {
			return nil, util.ErrEOF
		}
		return t.mmap[off : off+size], nil
	}
...
func (t *Table) readNoFail(off, size int) []byte {
	res, err := t.read(off, size)
	util.Check(err)
	return res
}

func (t *Table) block(idx int) (block, error) {
	util.AssertTruef(idx >= 0, "idx=%d", idx)
