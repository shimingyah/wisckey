...
// See Github issue: https://github.com/shimingyah/wisckey/issues/315
...
func (item *Item) yieldItemValue() ([]byte, func(), error) {
	...
	for {
		...
		runCallback(cb)
		// Do not put wisckeyMove on the left in append. It seems to cause some sort of manipulation.
		keyTs := y.KeyWithTs(item.Key(), item.Version())
		key = make([]byte, len(wisckeyMove)+len(keyTs))
		n := copy(key, wisckeyMove)
		copy(key[n:], keyTs)
...
// IteratorOptions is used to set options when iterating over Wisckey key-value
// stores.
//
...
type IteratorOptions struct {
	...
	internalAccess bool // Used to allow internal access to wisckey keys.
}
...
// DefaultIteratorOptions contains default options when iterating over Wisckey key-value stores.
...
func (it *Iterator) parseItem() bool {
	...
	// Skip wisckey keys.
	if !it.opt.internalAccess && bytes.HasPrefix(key, wisckeyPrefix) {
		mi.Next()
		return false
	}
