...
WisckeyDB is an embeddable, persistent and fast key-value (KV) database
 ...
 * [Getting Started](#getting-started)
  ...
  * [Other Projects Using Wisckey](#other-projects-using-wisckey)
...
### Opening a database
The top-level object in Wisckey is a `DB`. It represents multiple files on disk
in specific directories, which contain the data for a single database.

To open your database, use the `wisckey.Open()` function, with the appropriate
...
func main() {
  // Open the Wisckey database located in the /tmp/wisckey directory.
  // It will be created if it doesn't exist.
  opts := wisckey.DefaultOptions
  opts.Dir = "/tmp/wisckey"
  opts.ValueDir = "/tmp/wisckey"
...
Please note that Wisckey obtains a lock on the directories so multiple processes
...
```go
err := db.View(func(txn *wisckey.Txn) error {
  // Your code here…
  return nil
})
```

You cannot perform any writes or deletes within this transaction. Wisckey
...
```go
err := db.Update(func(txn *wisckey.Txn) error {
  // Your code here…
  return nil
})
```
...
returned. This is the recommended way to use Wisckey transactions.
...
Wisckey allows an optional callback to the `Txn.Commit()` method. Normally, the
...
Wisckey would lease a range of integers to hand out from memory, with the
...
too high would result in wasted integers if Wisckey is closed or crashes.
To avoid wasted integers, call `Release` before closing Wisckey.
...
### Merge Operations
Wisckey provides support for unordered merge operations. You can define a func
...
Wisckey allows setting an optional Time to Live (TTL) value on keys. Once the TTL has
...
```go
err := db.View(func(txn *wisckey.Txn) error {
  opts := wisckey.DefaultIteratorOptions
...
By default, Wisckey prefetches the values of the next 100 items. You can adjust
...
```go
db.View(func(txn *wisckey.Txn) error {
  it := txn.NewIterator(wisckey.DefaultIteratorOptions)
  defer it.Close()
  prefix := []byte("1234")
...
#### Key-only iteration
Wisckey supports a unique mode of iteration called _key-only_ iteration. It is
...
### Stream
Wisckey provides a Stream framework, which concurrently iterates over all or a
...
Wisckey. This is a lot faster way to iterate over Wisckey than using a single
Iterator. Stream supports Wisckey in both managed and normal mode.
...
getting ported over to Wisckey.
...
stream.LogPrefix = "Wisckey.Streaming" // For identifying stream logs. Outputs to Logger.

// ChooseKey is called concurrently for every key. If left nil, assumes true by default.
stream.ChooseKey = func(item *wisckey.Item) bool {
  return bytes.HasSuffix(item.Key(), []byte("er"))
}

// KeyToList is called concurrently for chosen keys. This can be used to convert
// Wisckey data into custom key-values. If nil, uses stream.ToList, a default
...
### Garbage Collection
Wisckey values need to be garbage collected, because of two reasons:

* Wisckey keeps values separately from the LSM tree. This means that the compaction operations
...
Wisckey relies on the client to perform garbage collection at a time of their choosing. It provides
...
* `DB.RunValueLogGC()`: This method is designed to do garbage collection while
  Wisckey is online. Along with randomly picking a file, it uses statistics generated by the
...
* `DB.PurgeOlderVersions()`: This method is **DEPRECATED** since v1.5.0. Now, Wisckey's LSM tree automatically discards older/invalid versions of keys.
...
used to do online backups and restores. Wisckey v0.9 provides a CLI tool
`wisckey`, which can do offline backup/restore. Make sure you have `$GOPATH/bin`
in your PATH to use this tool.

The command below will create a version-agnostic backup of the database, to a
file `wisckey.bak` in the current working directory

```
wisckey backup --dir <path/to/wisckeydb>
```

To restore `wisckey.bak` in the current working directory to a new database:

```
wisckey restore --dir <path/to/wisckeydb>
```

See `wisckey --help` for more details.

If you have a Wisckey database that was created using v0.8 (or below), you can
use the `wisckey_backup` tool provided in v0.8.1, and then restore it using the
command above to upgrade your database to work with the latest version.

```
wisckey_backup --dir <path/to/wisckeydb> --backup-file wisckey.bak
```

We recommend all users to use the `Backup` and `Restore` APIs and tools. However,
Wisckey is also rsync-friendly because all files are immutable, barring the
...
set -o history
set -o histexpand
# Makes a complete copy of a Wisckey database directory.
...
### Memory usage
Wisckey's memory usage can be managed by tweaking several options available in
...
If you want to decrease the memory usage of Wisckey instance, tweak these
options (ideally one at a time) until you achieve the desired
memory usage.

### Statistics
Wisckey records metrics using the [expvar] package, which is included in the Go
...
### Blog Posts
1. [Introducing Wisckey: A fast key-value store written natively in
Go](https://open.dgraph.io/post/wisckey/)
2. [Make Wisckey crash resilient with ALICE](https://blog.dgraph.io/post/alice/)
3. [Wisckey vs LMDB vs BoltDB: Benchmarking key-value databases in Go](https://blog.dgraph.io/post/wisckey-lmdb-boltdb/)
4. [Concurrent ACID Transactions in Wisckey](https://blog.dgraph.io/post/wisckey-txn/)

## Design
Wisckey was written with these design goals in mind:

- Write a key-value database in pure Go.
- Use latest research to build the fastest KV database for data sets spanning terabytes.
- Optimize for SSDs.

Wisckey’s design is based on a paper titled _[WiscKey: Separating Keys from
Values in SSD-conscious Storage][wisckey]_.

[wisckey]: https://www.usenix.org/system/files/conference/fast16/fast16-papers-lu.pdf

### Comparisons
| Feature                       | Wisckey                                    | RocksDB                       | BoltDB    |
| ----------------------------- | ------------------------------------------ | ----------------------------- | --------- |
...
<sup>1</sup> The [WISCKEY paper][wisckey] (on which Wisckey is based) saw big
...
<sup>3</sup> SSI: Serializable Snapshot Isolation. For more details, see the blog post [Concurrent ACID Transactions in Wisckey](https://blog.dgraph.io/post/wisckey-txn/)

<sup>4</sup> Wisckey provides direct access to value versions via its Iterator API.
...
The following is true for users on Wisckey v1.x.

This can happen if a long running iteration with `Prefetch` is set to false, but
a `Item::Value` call is made internally in the loop. That causes Wisckey to
...
1. Set `Prefetch` to true. Wisckey would then copy over the value and release the
...
If you're using Wisckey with `SyncWrites=false`, then your writes might not be written to value log
