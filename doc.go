/*
Package wisckey implements an embeddable, simple and fast key-value database,
written in pure Go. It is designed to be highly performant for both reads and
writes simultaneously. Wisckey uses Multi-Version Concurrency Control (MVCC), and
supports transactions. It runs transactions concurrently, with serializable
snapshot isolation guarantees.

Wisckey uses an LSM tree along with a value log to separate keys from values,
...
Wisckey has the following main types: DB, Txn, Item and Iterator. DB contains
