...
func (r keyRange) overlapsWith(dst keyRange) bool {
	...
	if util.CompareKeys(r.left, dst.right) > 0 {
		return false
	}
	// If my right is less than dst left, we have no overlap.
	if util.CompareKeys(r.right, dst.left) < 0 {
		return false
	}
	// We have overlap.
	return true
...
func getKeyRange(tables []*table.Table) keyRange {
	...
	for i := 1; i < len(tables); i++ {
		if util.CompareKeys(tables[i].Smallest(), smallest) < 0 {
			smallest = tables[i].Smallest()
		}
		if util.CompareKeys(tables[i].Biggest(), biggest) > 0 {
			biggest = tables[i].Biggest()
		}
	}
	return keyRange{
		left:  util.KeyWithTs(util.ParseKey(smallest), math.MaxUint64),
		right: util.KeyWithTs(util.ParseKey(biggest), 0),
	}
}
...
func (cs *compactStatus) compareAndAdd(_ thisAndNextLevelRLocked, cd compactDef) bool {
	...
	util.AssertTruef(level < len(cs.levels)-1, "Got level %d. Max levels: %d", level, len(cs.levels))
	thisLevel := cs.levels[level]
