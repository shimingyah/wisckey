...
// Opens a wisckey db and runs a a test on it.
func runWisckeyTest(t *testing.T, opts *Options, test func(t *testing.T, db *DB)) {
	dir, err := ioutil.TempDir(".", "wisckey-test")
	require.NoError(t, err)
...
func TestWrite(t *testing.T) {
	runWisckeyTest(t, nil, func(t *testing.T, db *DB) {
		for i := 0; i < 100; i++ {
...
func TestForceCompactL0(t *testing.T) {
	dir, err := ioutil.TempDir("", "wisckey")
	require.NoError(t, err)
	defer os.RemoveAll(dir)
...
func TestIterateParallel(t *testing.T) {
	...
	opt := DefaultOptions
	runWisckeyTest(t, &opt, func(t *testing.T, db *DB) {
		var wg sync.WaitGroup
		var txns []*Txn
		for i := 0; i < N; i++ {
			wg.Add(1)
...
func TestPidFile(t *testing.T) {
	runWisckeyTest(t, nil, func(t *testing.T, db *DB) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "Another process is using this Wisckey database")
	})
}
...
func TestInvalidKey(t *testing.T) {
	runWisckeyTest(t, nil, func(t *testing.T, db *DB) {
		err := db.Update(func(txn *Txn) error {
			err := txn.Set([]byte("!wisckey!head"), nil)
			require.Equal(t, ErrInvalidKey, err)

			err = txn.Set([]byte("!wisckey!"), nil)
			require.Equal(t, ErrInvalidKey, err)

			err = txn.Set([]byte("!wisckey"), []byte("WisckeyDB"))
			require.NoError(t, err)
			return err
		})
		require.NoError(t, err)
		...
		require.NoError(t, db.View(func(txn *Txn) error {
			item, err := txn.Get([]byte("!wisckey"))
			if err != nil {
				return err
			}
			require.NoError(t, item.Value(func(val []byte) error {
				require.Equal(t, []byte("WisckeyDB"), val)
				return nil
			}))
			return nil
		}))
...
// Earlier, if head is not pointing to latest Vlog file, then at replay wisckey used to crash with
// index out of range panic. After fix in this commit it should not.
func TestNoCrash(t *testing.T) {
	dir, err := ioutil.TempDir("", "wisckey")
	require.NoError(t, err, "cannot create wisckey dir")
	defer os.RemoveAll(dir)
...
func TestForceFlushMemtable(t *testing.T) {
	dir, err := ioutil.TempDir("", "wisckey")
	require.NoError(t, err, "temp dir for wisckey count not be created")
