...
type Options struct {
	// 1. Mandatory flags
	// -------------------
	// Directory to store the data in. If it doesn't exist, Wisckey will
	// try to create it for you.
	Dir string
	// Directory to store the value log in. Can be the same as Dir. If it
	// doesn't exist, Wisckey will try to create it for you.
	ValueDir string
	...
	// This is only useful for databases built on top of Wisckey (like Dgraph).
...
// log, and make Wisckey act more like a typical LSM tree.
...
func init() {
	...
	// size of key-value a user can pass to Wisckey. For e.g., if we set
