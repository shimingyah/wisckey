...
func (m *Manifest) clone() Manifest {
	...
	ret := createManifest()
	util.Check(applyChangeSet(&ret, &changeSet))
	return ret
}

// openOrCreateManifestFile opens a Wisckey manifest file if it exists, or creates on if
// one doesn’t.
...
func helpOpenOrCreateManifestFile(dir string, readOnly bool, deletionsThreshold int) (ret *manifestFile, result Manifest, err error) {
	...
	// We explicitly sync in addChanges, outside the lock.
	fd, err := util.OpenExistingFile(path, flags)
	if err != nil {
		...
		m := createManifest()
		fd, netCreations, err := helpRewrite(dir, &m)
		if err != nil {
			return nil, Manifest{}, err
		}
		util.AssertTrue(netCreations == 0)
		mf := &manifestFile{
			fd:                        fd,
			dir:                       dir,
			manifest:                  m.clone(),
	...
	manifest, truncOffset, err := ReplayManifestFile(fd)
	if err != nil {
		fd.Close()
		return nil, Manifest{}, err
	}
...
func (mf *manifestFile) addChanges(changesParam []*pb.ManifestChange) error {
	...
	if mf.manifest.Deletions > mf.deletionsRewriteThreshold &&
		...
		binary.BigEndian.PutUint32(lenCrcBuf[4:8], crc32.Checksum(buf, util.CastagnoliCrcTable))
...
func helpRewrite(dir string, m *Manifest) (*os.File, int, error) {
	...
	// We explicitly sync.
	fd, err := util.OpenTruncFile(rewritePath, false)
	if err != nil {
		return nil, 0, err
	}
	...
	binary.BigEndian.PutUint32(lenCrcBuf[4:8], crc32.Checksum(changeBuf, util.CastagnoliCrcTable))
	...
	fd, err = util.OpenExistingFile(manifestPath, 0)
	if err != nil {
		return nil, 0, err
	}
	if _, err := fd.Seek(0, io.SeekEnd); err != nil {
	...
	if err := util.SyncDir(dir); err != nil {
		fd.Close()
		return nil, 0, err
	}

	return fd, netCreations, nil
}
...
func (mf *manifestFile) rewrite() error {
	...
	fd, netCreations, err := helpRewrite(mf.dir, &mf.manifest)
	if err != nil {
		return err
	}
	mf.fd = fd
...
func ReplayManifestFile(fp *os.File) (ret Manifest, truncOffset int64, err error) {
	...
	for {
		...
		if crc32.Checksum(buf, util.CastagnoliCrcTable) != binary.BigEndian.Uint32(lenCrcBuf[4:8]) {
			break
		}
