...
func (vlog *valueLog) rewrite(f *logFile, tr trace.Trace) error {
	...
	fe := func(e Entry) error {
		...
		if vp.Fid == f.fid && vp.Offset == e.offset {
			...
			if bytes.HasPrefix(e.Key, wisckeyMove) {
				ne.Key = append([]byte{}, e.Key...)
			} else {
				ne.Key = make([]byte, len(wisckeyMove)+len(e.Key))
				n := copy(ne.Key, wisckeyMove)
				copy(ne.Key[n:], e.Key)
			}
...
func (vlog *valueLog) deleteMoveKeysFor(fid uint32, tr trace.Trace) error {
	...
	err := db.View(func(txn *Txn) error {
		...
		for itr.Seek(wisckeyMove); itr.ValidForPrefix(wisckeyMove); itr.Next() {
			count++
			item := itr.Item()
...
func (vlog *valueLog) open(db *DB, ptr valuePointer, replayFn logEntry) error {
	...
	vlog.db = db
	vlog.elog = trace.NewEventLog("Wisckey", "Valuelog")
...
// 仅仅是写入value log，memtab的写入不在此部分
// 先把同一个请求的所有entry写入buf，此时会拷贝key、value的值，然后再统一append 磁盘。
...
func (vlog *valueLog) runGC(discardRatio float64, head valuePointer) error {
	...
	case vlog.garbageCh <- struct{}{}:
		...
		tr := trace.New("Wisckey.ValueLog", "GC")
		tr.SetMaxEvents(100)
		defer func() {
			tr.Finish()
			<-vlog.garbageCh
...
// This will be called while storing stats in WisckeyDB
