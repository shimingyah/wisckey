...
func newOracle(opt Options) *oracle {
	orc := &oracle{
		...
		readMark: &y.WaterMark{Name: "wisckey.PendingReads"},
		txnMark:  &y.WaterMark{Name: "wisckey.TxnTimestamp"},
...
// Txn represents a Wisckey transaction.
type Txn struct {
	readTs   uint64
	commitTs uint64
...
func (txn *Txn) modify(e *Entry) error {
	...
	case len(e.Key) == 0:
		return ErrEmptyKey
	case bytes.HasPrefix(e.Key, wisckeyPrefix):
		return ErrInvalidKey
	case len(e.Key) > maxKeySize:
		// Key length can't be more than uint16, as determined by table::header.  To
		// keep things safe and allow wisckey move prefix and a timestamp suffix, let's
...
func (txn *Txn) commitAndSend() (func() error, error) {
	...
	// 在加入一个key为"!wisckey!txn"的entry，标识这个事物的结束。
	// log.Printf("%s\n", b.String())
	e := &Entry{
...
//
// 5. If callback is provided, Wisckey will return immediately after checking
...
// NewTransaction creates a new transaction. Wisckey supports concurrent execution of transactions,
// providing serializable snapshot isolation, avoiding write skews. Wisckey achieves this by tracking
...
func (db *DB) newTransaction(update, isManaged bool) *Txn {
	...
	//    would be detected.
	// See issue: https://github.com/shimingyah/wisckey/issues/574
	if !isManaged {
