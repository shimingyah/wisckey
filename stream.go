...
// Stream provides a framework to concurrently iterate over a snapshot of Wisckey, pick up
...
type Stream struct {
	...
	// Wisckey would produce log entries in Infof to indicate the progress of Stream. LogPrefix can
	// be used to help differentiate them from other activities. Default is "Wisckey.Stream".
	LogPrefix string
...
func (st *Stream) Orchestrate(ctx context.Context) error {
	...
	// Picks up ranges from Wisckey, and sends them to rangeCh.
...
func (db *DB) newStream() *Stream {
	return &Stream{db: db, NumGo: 16, LogPrefix: "Wisckey.Stream"}
}
