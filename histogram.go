...
func (db *DB) buildHistogram(keyPrefix []byte) *sizeHistogram {
	...
	wisckeyHistogram := newSizeHistogram()

	// Collect key and value sizes.
	for itr.Seek(keyPrefix); itr.ValidForPrefix(keyPrefix); itr.Next() {
		item := itr.Item()
		wisckeyHistogram.keySizeHistogram.Update(item.KeySize())
		wisckeyHistogram.valueSizeHistogram.Update(item.ValueSize())
	}
	return wisckeyHistogram
}
