...
import (
	...
	"github.com/shimingyah/wisckey/options"
	"github.com/shimingyah/wisckey/pb"
	"github.com/shimingyah/wisckey/y"
	"github.com/spf13/cobra"
)

var testCmd = &cobra.Command{
	Use:   "bank",
	Short: "Run bank test on Wisckey.",
	Long: `
This command runs bank test on Wisckey, inspired by Jepsen. It creates many
...
var bankTest = &cobra.Command{
	Use:   "test",
	Short: "Execute bank test on Wisckey.",
	RunE:  runTest,
}
...
var bankDisect = &cobra.Command{
	...
	Long: `
Disect the bank output WisckeyDB to find the first transaction which causes
...
func init() {
	...
	bankTest.Flags().BoolVarP(&checkStream, "check_stream", "s", false,
		"If true, the test will send transactions to another wisckey instance via the stream "+
...
func getBalance(txn *wisckey.Txn, account int) (uint64, error) {
...
func putBalance(txn *wisckey.Txn, account int, bal uint64) error {
...
func moveMoney(db *wisckey.DB, from, to int) error {
	return db.Update(func(txn *wisckey.Txn) error {
...
func seekTotal(txn *wisckey.Txn) ([]account, error) {
...
func findFirstInvalidTxn(db *wisckey.DB, lowTs, highTs uint64) uint64 {
	...
	err := checkAt(midTs)
	if err == wisckey.ErrKeyNotFound || err == nil {
...
func compareTwo(db *wisckey.DB, before, after uint64) {
...
func runDisect(cmd *cobra.Command, args []string) error {
	...
	db, err := wisckey.OpenManaged(opts)
	if err != nil {
		return err
	}
	...
	var min, max uint64 = math.MaxUint64, 0
	{
		...
		iopt := wisckey.DefaultIteratorOptions
...
func runTest(cmd *cobra.Command, args []string) error {
	...
	if checkStream {
		...
		go func() {
			...
			for range ticker.C {
				...
				y.Check(tmpDb.View(func(txn *wisckey.Txn) error {
					_, err := seekTotal(txn)
					if err != nil {
	...
	go func() {
		...
		for range ticker.C {
			...
			y.Check(db.View(func(txn *wisckey.Txn) error {
				_, err := seekTotal(txn)
				if err != nil {
