...
func init() {
	...
	infoCmd.Flags().BoolVar(&opt.showKeys, "show-keys", false, "Show keys stored in Wisckey")
...
var infoCmd = &cobra.Command{
	Use:   "info",
	Short: "Health info about Wisckey database.",
	Long: `
This command prints information about the wisckey key-value store.  It reads MANIFEST and prints its
info. It also prints info about missing/extra files, and general information about the value log
files (which are not referenced by the manifest).  Use this tool to report any issues about Wisckey
to the Dgraph team.
`,
	RunE: handleInfo,
}
...
func showKeys(db *wisckey.DB, prefix string) error {
	if len(prefix) > 0 {
...
func lookup(db *wisckey.DB) error {
	txn := db.NewTransaction(false)
	defer txn.Discard()
	...
	iopts := wisckey.DefaultIteratorOptions
...
func printKey(item *wisckey.Item, showValue bool) error {
...
func tableInfo(dir, valueDir string, db *wisckey.DB) {
...
func printInfo(dir, valueDir string) error {
	...
	fp, err := os.Open(filepath.Join(dir, wisckey.ManifestFilename))
	if err != nil {
		return err
	}
	defer func() {
	...
	manifest, truncOffset, err := wisckey.ReplayManifestFile(fp)
	if err != nil {
		return err
	}
	fp.Close()
	...
	manifestInfo, ok := fileinfoByName[wisckey.ManifestFilename]
	if ok {
		fileinfoMarked[wisckey.ManifestFilename] = true
		truncatedString := ""
