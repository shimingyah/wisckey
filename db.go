...
import (
	...
	"github.com/shimingyah/wisckey/skl"
	"github.com/shimingyah/wisckey/table"
	"github.com/shimingyah/wisckey/y"
)

var (
	wisckeyPrefix     = []byte("!wisckey!")        // Prefix for internal keys used by wisckey.
	head              = []byte("!wisckey!head")    // For storing value offset for replay.
	txnKey            = []byte("!wisckey!txn")     // For indicating end of entries in txn.
	wisckeyMove       = []byte("!wisckey!move")    // For key-value pairs which got moved during GC.
	lfDiscardStatsKey = []byte("!wisckey!discard") // For storing lfDiscardStats
)
...
// DB provides the various functions required to interact with wisckey.
// DB is thread-safe.
type DB struct {
...
func Open(opt Options) (db *DB, err error) {
	...
	db = &DB{
		...
		opt:           opt,
		manifest:      manifestFile,
		elog:          trace.NewEventLog("Wisckey", "DB"),
...
// need to write "old versions after new versions" to the wisckeyMove keyspace. Thus, for normal
...
func (db *DB) get(key []byte) (y.ValueStruct, error) {
	...
	var version uint64
	if bytes.HasPrefix(key, wisckeyMove) {
		// If we are checking wisckeyMove key, we should look into all the
...
// batchSet applies a list of wisckey.Entry. If a request level error occurs it
// will be returned.
...
func (db *DB) handleFlushTask(ft flushTask) error {
	...
	// Store wisckey head even if vptr is zero, need it for readTs
...
// Sequence represents a Wisckey sequence.
...
func (db *DB) KeySplits(prefix []byte) []string {
	...
	for _, ti := range db.Tables(false) {
		// We don't use ti.Left, because that has a tendency to store !wisckey
		// keys.
...
// DropAll would drop all the data stored in Wisckey. It does this in the following way.
// - Stop accepting new writes.
